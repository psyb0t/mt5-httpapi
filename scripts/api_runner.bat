@echo off
:: api_runner.bat  broker  account  port  token  utc_offset  mode
...
echo [%DATE% %TIME%] [api:!AR_BROKER!/!AR_ACCOUNT!] === PROCESS STARTED on port !AR_PORT! (utc_offset=!AR_OFFSET! mode=!AR_MODE!) === >> "!AR_LOG!"
echo [%DATE% %TIME%] [start] [api:!AR_BROKER!/!AR_ACCOUNT!] PROCESS STARTED on port !AR_PORT! utc_offset=!AR_OFFSET! mode=!AR_MODE! >> "%FULL_LOG%"

cd /d "%SHARED%"
"%PYDIR%\python.exe" -m mt5api --broker !AR_BROKER! --account !AR_ACCOUNT! --port !AR_PORT! --token "!AR_TOKEN!" --utc-offset "!AR_OFFSET!" --mode "!AR_MODE!" >> "!AR_LOG!" 2>&1
