...
Add-Content -Path $logFile -Value ("[" + (Get-Date).ToString('s') + "] [tailer] starting (pid=$PID, since=$since)")
    ...
    try {
        Get-WinEvent -FilterHashtable @{
            ...
            ForEach-Object {
                ...
                "[$ts] [$log/$lvl] [$prov] $id $msg"
            } |
            Add-Content -Path $logFile
    } catch {
        Add-Content -Path $logFile -Value ("[" + (Get-Date).ToString('s') + "] [tailer] poll error: $_")
    }
    $since = $now
